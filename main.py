...
import config
from Scripts.raw_data_processing_scripts import movie_tv_dml_generator, person_dml_generator, aka_dml_generator
...
def main():
    ...
    #try:
    #    aka_dml_generator.generate_aka_dml(config.db_config,config.data_file_path,config.output_file_path['aka_output_path'],'output_aka_dml',50000)
    #except Exception as e:
    #    print(f"Error generating aka DML: {e}")

    try:
        populator.populate(config.output_file_path['aka_output_path'], config.db_config)
    except Exception as e:
        print(f"Error populating person data: {e}")
